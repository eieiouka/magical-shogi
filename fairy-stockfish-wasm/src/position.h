...
public:
  ...
  bool magical_shogi() const;
  char magical_role(PieceType pt) const;
  bool magical_shot(Move m) const;
  bool magical_swap(Move m) const;
  bool magical_jump(Move m) const;
  Square magical_capture_square(Move m) const;
...
inline bool Position::magical_shogi() const {
  return var && var->magicalShogi;
}

inline char Position::magical_role(PieceType pt) const {
  if (!magical_shogi() || pt == NO_PIECE_TYPE)
      return 0;
  char ch = char(std::tolower(piece_to_char()[make_piece(WHITE, pt)]));
  // Promoted identities a-e retain the corresponding character magic.
  return ch == 'a' ? 'n' : ch == 'b' ? 'h' : ch == 'c' ? 'i'
       : ch == 'd' ? 's' : ch == 'e' ? 'm' : ch;
}

inline bool Position::magical_shot(Move m) const {
  if (!magical_shogi() || magical_role(type_of(moved_piece(m))) != 'n')
      return false;
  Square from = from_sq(m), to = to_sq(m);
  int df = int(file_of(to)) - int(file_of(from));
  int dr = int(rank_of(to)) - int(rank_of(from));
  int forward = color_of(moved_piece(m)) == WHITE ? 2 : -2;
  return dr == forward && (df == 0 || std::abs(df) == 2);
}

inline bool Position::magical_swap(Move m) const {
  return type_of(m) == SPECIAL && magical_role(type_of(moved_piece(m))) == 'i';
}

inline bool Position::magical_jump(Move m) const {
  return type_of(m) == SPECIAL && magical_role(type_of(moved_piece(m))) == 'm';
}

inline Square Position::magical_capture_square(Move m) const {
  if (magical_shot(m))
      return to_sq(m);
  if (magical_jump(m))
      return Square((int(from_sq(m)) + int(to_sq(m))) / 2);
  return to_sq(m);
}
...
inline bool Position::capture_or_promotion(Move m) const {
  ...
  return type_of(m) == PROMOTION || type_of(m) == EN_PASSANT
      || (type_of(m) == SPECIAL && magical_shogi() && (magical_shot(m) || magical_jump(m)))
...
inline bool Position::capture(Move m) const {
  ...
  if (type_of(m) == SPECIAL && magical_shogi())
      return magical_shot(m) || magical_jump(m);
