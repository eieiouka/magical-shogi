...
Bitboard Position::attackers_to(Square s, Bitboard occupied, Color c, Bitboard janggiCannons) const {
  ...
  // Margo's jump assassination attacks the jumped square, not the landing
  // square.  It is available sideways and backwards only, and only if the
  // square beyond the victim is empty.
  if (magical_shogi())
  {
      // Hanna's two-square float is move-only. The generic fairy attack table
      // contains its Dabbaba destination, so remove it from capture attacks.
      // Unpromoted Sherry's two-square advance is move-only as well.
      Bitboard falseAttackers = b;
      while (falseAttackers)
      {
          Square from = pop_lsb(falseAttackers);
          PieceType pt = type_of(piece_on(from));
          char role = magical_role(pt);
          int df = int(file_of(s)) - int(file_of(from));
          int dr = int(rank_of(s)) - int(rank_of(from));
          int twoForward = c == WHITE ? 2 : -2;
          bool hannaFloat = role == 'h' && df == 0 && dr == twoForward;
          bool plainSherryLong = role == 's'
                               && var->promotedPieceType[pt] != NO_PIECE_TYPE
                               && df == 0 && dr == twoForward;
          bool blockedNanokaShot = role == 'n' && dr == twoForward
                                  && (df == 0 || std::abs(df) == 2)
                                  && (occupied & make_square(File((int(file_of(from)) + int(file_of(s))) / 2),
                                                               Rank((int(rank_of(from)) + int(rank_of(s))) / 2)));
          if (hannaFloat || plainSherryLong || blockedNanokaShot)
              b ^= from;
      }
      ...
      {
          PieceType pt = pop_lsb(ps);
          if (magical_role(pt) != 'm')
              continue;
          Bitboard margos = pieces(c, pt) & occupied;
          while (margos)
          {
              Square from = pop_lsb(margos);
              int df = int(file_of(s)) - int(file_of(from));
              int dr = int(rank_of(s)) - int(rank_of(from));
              int back = c == WHITE ? -1 : 1;
              if (!((dr == 0 && std::abs(df) == 1)
                    || (dr == back && std::abs(df) <= 1)))
                  continue;
              int lf = int(file_of(s)) + df;
              int lr = int(rank_of(s)) + dr;
              if (lf < int(FILE_A) || lf > int(max_file()) || lr < int(RANK_1) || lr > int(max_rank()))
                  continue;
              Square landing = make_square(File(lf), Rank(lr));
              if (!(occupied & landing))
                  b |= from;
          }
      }
...
bool Position::legal(Move m) const {
  ...
  // Magical moves can keep the mover at its source, exchange two friendly
  // pieces, or capture on the midpoint. Build the exact resulting occupancy
  // before testing whether Emma remains attacked.
  if (magical_shogi() && count<KING>(us)
      && (magical_shot(m) || type_of(m) == SPECIAL))
  {
      Bitboard occupied = pieces();
      if (magical_shot(m))
          occupied ^= to;
      else if (magical_jump(m))
          occupied = (occupied ^ from ^ magical_capture_square(m)) | to;
      // Hiro swaps two occupied friendly squares, so occupancy is unchanged.
      // If the adjacent ally is Emma, her royal square moves to Hiro's source.
      Square ksq = magical_swap(m) && type_of(piece_on(to)) == KING
                 ? from : square<KING>(us);
      return !(attackers_to(ksq, occupied, ~us) & occupied);
  }
...
bool Position::pseudo_legal(const Move m) const {
  ...
  if (checkers() && !(checkers() & non_sliding_riders()))
  {
      if (type_of(pc) != KING)
      {
          ...
          Square king = square<KING>(us);
          bool capturesChecker = bool(checkers() & to);
          bool blocksSlider = bool(between_bb(king, checksq) & to);
          bool blocksNanoka = false;
          if (magical_shogi() && magical_role(type_of(piece_on(checksq))) == 'n')
          {
              Square midpoint = make_square(File((int(file_of(checksq)) + int(file_of(king))) / 2),
                                            Rank((int(rank_of(checksq)) + int(rank_of(king))) / 2));
              blocksNanoka = to == midpoint;
          }
          bool leaperCheck = bool(LeaperAttacks[~us][type_of(piece_on(checksq))][checksq] & king);
          if (!(capturesChecker || blocksSlider || blocksNanoka)
              || (leaperCheck && !capturesChecker && !blocksNanoka))
              return false;
      }
...
void Position::do_move(Move m, StateInfo& newSt, bool givesCheck) {
  ...
  Square to = to_sq(m);
  Piece pc = moved_piece(m);
  const bool magicShot = magical_shot(m);
  const bool magicSwap = magical_swap(m);
  const bool magicJump = magical_jump(m);
  const Square magicCapsq = (magicShot || magicJump) ? magical_capture_square(m) : to;
  Piece captured = magicSwap ? NO_PIECE
                 : piece_on(type_of(m) == EN_PASSANT ? capture_square(to)
                           : (magicShot || magicJump) ? magicCapsq : to);
  if (to == from)
  {
      assert((type_of(m) == PROMOTION && sittuyin_promotion()) || (is_pass(m) && (pass(us) || var->wallOrMove )));
      captured = NO_PIECE;
  }
  st->capturedpromoted = captured && is_promoted(magicCapsq);
  st->unpromotedCapturedPiece = captured ? unpromoted_piece_on(magicCapsq) : NO_PIECE;
  st->captureSquare = magicCapsq;
  st->promotionPawn = NO_PIECE;
  st->pass = is_pass(m);

  assert(color_of(pc) == us);
  assert(captured == NO_PIECE || color_of(captured) == (type_of(m) != CASTLING ? them : us));
  assert(!magicSwap || (piece_on(to) && color_of(piece_on(to)) == us));
  ...
  if (captured)
  {
      Square capsq = (magicShot || magicJump) ? magicCapsq : to;
  ...
  {
      // Nanoka stays on the source square; only the captured piece changes.
  }
  else if (magicSwap)
  {
      Piece ally = piece_on(to);
      k ^= Zobrist::psq[pc][from] ^ Zobrist::psq[pc][to]
         ^ Zobrist::psq[ally][to] ^ Zobrist::psq[ally][from];
  }
  else
  {
      k ^= Zobrist::psq[pc][from] ^ Zobrist::psq[pc][to];
  ...
  else if (type_of(m) != CASTLING && !magicShot)
  {
      if (Eval::useNNUE)
      {
          dp.piece[0] = pc;
      ...
      {
          Piece ally = piece_on(to);
          bool pcPromoted = is_promoted(from);
          bool allyPromoted = is_promoted(to);
          Piece pcBase = unpromoted_piece_on(from);
          Piece allyBase = unpromoted_piece_on(to);
          remove_piece(to);
          remove_piece(from);
          put_piece(pc, to, pcPromoted, pcBase);
          put_piece(ally, from, allyPromoted, allyBase);
          if (Eval::useNNUE)
          {
              dp.dirty_num = 2;
              dp.piece[1] = ally;
  ...
  if (type_of(pc) == PAWN)
  {
      ...
      st->pawnKey ^= (type_of(m) != DROP ? Zobrist::psq[pc][from] : 0) ^ Zobrist::psq[pc][to];
           ...
           || ((magicSwap || magicJump) && !is_promoted(to)
               ...
               && (promotion_zone(us) & to)))
  {
      ...
      put_piece(promotion, to, true,
                (type_of(m) == PIECE_PROMOTION || magicSwap || magicJump) ? pc : NO_PIECE);
  ...
  if (magical_shogi())
      st->checkersBB = attackers_to(square<KING>(them), us) & pieces(us);
  else
      st->checkersBB = givesCheck ? attackers_to(square<KING>(them), us) & pieces(us) : Bitboard(0);
  assert(magical_shogi() || givesCheck == bool(st->checkersBB));
...
void Position::undo_move(Move m) {
  ...
  Square from = from_sq(m);
  Square to = to_sq(m);
  const bool magicShot = magical_shogi() && type_of(m) != DROP && !empty(from) && magical_shot(m);
  char magicRole = 0;
  if (magical_shogi() && type_of(m) == SPECIAL)
  {
      if (!empty(to))
          magicRole = magical_role(type_of(piece_on(to)));
  }
  const bool magicSwap = magicRole == 'i';
  const bool magicJump = magicRole == 'm';
  Piece pc = magicShot ? piece_on(from) : piece_on(to);
  ...
  else if (type_of(m) == PIECE_PROMOTION || ((magicSwap || magicJump) && st->promotionPawn))
  {
  ...
  else
  {
      ...
      {
          // The shooter never moved.
      }
      else if (magicSwap)
      {
          Piece ally = piece_on(from);
          bool pcPromoted = is_promoted(to);
          bool allyPromoted = is_promoted(from);
          Piece pcBase = unpromoted_piece_on(to);
          Piece allyBase = unpromoted_piece_on(from);
          remove_piece(from);
          remove_piece(to);
          put_piece(pc, from, pcPromoted, pcBase);
          put_piece(ally, to, allyPromoted, allyBase);
      }
      else
      ...
      if (st->capturedPiece)
      {
          Square capsq = (magicShot || magicJump) ? st->captureSquare : to;
...
Key Position::key_after(Move m) const {
  ...
  // This helper is only used for speculative TT prefetch. Magical SPECIAL
  // moves may capture on another square or swap friendly pieces, so the
  // ordinary incremental formula does not apply.
  if (magical_shogi() && type_of(m) == SPECIAL)
      return k;
