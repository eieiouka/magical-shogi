...
namespace {
    ...
    Variant* magical_shogi_variant() {
        const std::pair<const char*, const char*> entries[] = {
            {"magicalShogi", "true"}, {"maxRank", "6"}, {"maxFile", "6"},
            {"customPiece1", "n:mfsWcfsWfcDfcA"}, {"customPiece2", "h:fKmfD"},
            {"customPiece3", "i:fsW"}, {"king", "k:K"},
            {"customPiece4", "s:fmW2fcF"}, {"customPiece5", "m:fK"},
            {"customPiece6", "a:KfcDfcA"}, {"customPiece7", "b:KmfD"},
            {"customPiece8", "c:K"}, {"customPiece9", "d:KfW2"},
            {"customPiece10", "e:K"},
            {"pieceValueMg", "n:1150 h:1300 i:750 s:700 m:1500 a:1625 b:1775 c:1225 d:1175 e:1975"},
            {"pieceValueEg", "n:875 h:1300 i:750 s:700 m:1500 a:1350 b:1775 c:1225 d:1175 e:1975"},
            {"startFen", "mskihn/6/6/6/6/NHIKSM[-] w - - 0 1"},
            {"pieceDrops", "true"}, {"capturesToHand", "true"}, {"dropLoop", "false"},
            {"castling", "false"}, {"doubleStep", "false"},
            {"promotionRegionWhite", "*5 *6"}, {"promotionRegionBlack", "*1 *2"},
            {"promotionPawnTypes", "-"}, {"mandatoryPiecePromotion", "true"},
            {"promotedPieceType", "n:a h:b i:c s:d m:e"},
            {"dropRegionWhite", "*1 *2 *3 *4 *5"}, {"dropRegionBlack", "*2 *3 *4 *5 *6"},
            {"flagPiece", "k"}, {"flagRegionWhite", "*6"}, {"flagRegionBlack", "*1"},
            {"flagPieceSafe", "true"}, {"stalemateValue", "loss"}, {"checkmateValue", "loss"},
            {"nFoldRule", "3"}, {"nFoldValue", "draw"}, {"nMoveRule", "0"}, {"pocketSize", "5"}
        };
        Config config;
        for (const auto& entry : entries)
            config[entry.first] = entry.second;
        VariantParser<false> parser(config);
        return parser.parse(minishogi_variant_base()->init());
    }
...
void VariantMap::init() {
    ...
    add("magicalshogi", magical_shogi_variant());
#ifdef LARGEBOARDS
